...
<body>
    ...
    error_reporting(E_ERROR | E_PARSE); //giving warnings on undefined array keys, even though they are defined and working
        ...
        $trcountry = str_replace(' ', '', $country);//trimmed string
        $editcurl = curl_init();
        $url = 'http://127.0.0.1:5000';
        $headers = array(
            "Accept: text/xml",//defines to accept only xml response
        );
        $request_url = $url . '/' . $trcountry;
        echo $request_url;
        curl_setopt($editcurl,CURLOPT_URL,$request_url);
        curl_setopt($editcurl,CURLOPT_HEADER, $headers);
        curl_setopt($editcurl,CURLOPT_RETURNTRANSFER,true);
        curl_setopt($editcurl, CURLOPT_TIMEOUT, 10);
        $informationEdit = curl_exec($editcurl);
        $informationReceivedEdit = simplexml_load_string($informationEdit);
        echo $informationReceivedEdit;
        if(curl_errno($editcurl))
            print curl_error($editcurl);
        else
            curl_close($editcurl);
        ...
        $dataArr = array();
            ...
            $information =  $informationReceivedEdit->countryOverview;
            if(preg_match('/\bpopulation\b/', $trcountry)){
            ...
            else if(preg_match('/\bobesity\b/', $trcountry)){
            ...
            else if(preg_match('/\bhappiness\b/', $trcountry)){
